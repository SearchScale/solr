...
This acceleration is achieved with the help of the `cuvs` module in Solr. 
The `cuvs` module consists of a `CuVSCodec` that uses the `Lucene99AcceleratedHNSWVectorsFormat` from https://github.com/rapidsai/cuvs-lucene[cuVS-Lucene], a pluggable library for Apache Lucene.
cuVS-Lucene uses the https://arxiv.org/abs/2308.15136[CAGRA algorithm] for GPU based acceleration via the https://github.com/rapidsai/cuvs/tree/branch-25.10/[cuVS-Java API] from
the https://docs.rapids.ai/api/cuvs/stable/#[Nvidia's cuVS library] which is the GPU based vector search library.
https://developer.nvidia.com/cuda-downloads[CUDA 13.0+] and https://jdk.java.net/archive/[JDK 22] are required to use this feature.

Copy the module jar files
...
Define the fieldType in the schema 
...
Declare the xref:configuration-guide:codec-factory.adoc[codecFactory] in xref:configuration-guide:configuring-solrconfig-xml.adoc[solrconfig.xml]
...
[source,sh]
----
./bin/solr zk upconfig -n cuvs_vectors -d /tmp/cuvs_configset/conf && ./bin/solr create -c vectors -n cuvs_vectors
----
...
[source,sh]
----
curl -s "http://localhost:8983/solr/vectors/select?q=*:*&omitHeader=true"
----
