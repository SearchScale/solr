...
public class CuvsQParser extends AbstractVectorQParserBase {
    ...
    public Query parse() throws SyntaxError {
        ...
        if(qstr==null) throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "Missing float values ");

        List<?> vals = null;
        try {
            vals = (List<?>) Utils.fromJSONString(qstr);
        } catch (Exception e) {
            throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "Invalid format for value. should be a float[] "+qstr);
        }
        ...
        query = new GPUKnnFloatVectorQuery(localParams.get("f"),
                floats,
                parseIntVal("topK"),
                null,
    ...
    private int parseIntVal(String name) {
        ...
        if(s == null) {
            throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "Missing required localparam : "+ name);
        }
        try {
